...
- **会话** — 列表带筛选栏（项目 / 模型 / 日期 / Source / Originator）、可排序列（含 Source 列）、duration tag、KPI 行；详情页含 5 列 KPI、工具调用环形图、可展开消息预览；Codex 会话额外显示 Reasoning 令牌 KPI 与速率限制（5h/7d）
- **成本** — 日 / 周 / 月账单，异常日 markPoint 标记，按项目堆叠（Top 7 + 其他），CSV 导出
- **设置** — 显示偏好（紧凑数字 k/M/B 切换）+ 计费规则（按模型管理单价覆盖）

UI 顶栏支持深 / 浅色主题切换；偏好持久化到 localStorage。顶栏新增 Codex 速率限制徽章（有 Codex 数据时显示）。
